...
foreach ($options_list as $option_name => $arOption) {
	...
	if ($saveOption) {
		...
		if ($optionIsValid !== true) {
			$eeror_message .= 'ERROR: ' . Loc::getMessage('ISPRO_exchange2bitrixs_' . $option_name) . ' ' . $optionIsValid . PHP_EOL;
		}
	...
	if (($saveOption || $setDefault) && ($optionIsValid === true)) {
		...
		if (!setDefault) {
			$ok_message .= 'SAVED: ' . Loc::getMessage('ISPRO_exchange2bitrixs_' . $option_name) . PHP_EOL;
		}
	};
...
$tabList = [
	[
		'DIV' => 'description',
		'TAB' => Loc::getMessage('ISPRO_exchange2bitrixs_TAB_SET_DESC'),
		'ICON' => 'ib_settings',
		'TITLE' => Loc::getMessage('ISPRO_exchange2bitrixs_TAB_TITLE_DESC')
	],
	[
		'DIV' => 'setting',
		'TAB' => Loc::getMessage('ISPRO_exchange2bitrixs_TAB_SET_OPTION'),
		'ICON' => 'ib_settings',
		'TITLE' => Loc::getMessage('ISPRO_exchange2bitrixs_TAB_TITLE_OPTION')
	],
];
...
?>
<style>
	#exchange2bitrixs_form textarea {
		width: 100%;
		min-height: 150px;
	}
</style>
<form method="POST" action="<?= $currentUrl; ?>" enctype="multipart/form-data" id="exchange2bitrixs_form">
	<?= bitrix_sessid_post(); ?>
	<?
	$tabControl->Begin();
	?>
	...
	<tr>
		<td colspan="2">
			<?= BeginNote(); ?>
			<?= Loc::getMessage('ISPRO_exchange2bitrixs_DESCRIPTION'); ?>
			<?= EndNote(); ?>
		</td>
	</tr>
	...
	<?foreach ($options_list as $option_name => $arOption) :?>
		<tr>
			<td width="20%" valign="top">
				<?= Loc::getMessage('ISPRO_exchange2bitrixs_'.$option_name) ?>
			</td>
			<td width="80%">
				...
				<?elseif ($arOption['type'] == 'select') :?>
					...
					<? foreach ($arOption['values'] as $value) : ?>
						<option value="<?= $value ?>" <?= ($option[$option_name] == $value) ? 'selected' : '' ?>>
							<?= Loc::getMessage('ISPRO_exchange2bitrixs_'.$option_name.'_'.$value) ?>
						</option>
					<? endforeach ?>
					</select>
				<?else :?>
	...
	<tr>
		<td colspan="2">
			<input type="submit" class="adm-btn-save" name="save" value="<? echo Loc::getMessage('ISPRO_exchange2bitrixs_SAVE'); ?>">
			<input type="submit" class="adm-btn-save" name="reset" value="<? echo Loc::getMessage('ISPRO_exchange2bitrixs_DEFAULT'); ?>">
		</td>
	</tr>
